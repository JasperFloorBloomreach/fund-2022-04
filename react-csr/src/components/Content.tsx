...
export function Content(props: BrProps) {
  ...
  const {
    ...
    image: imageRef,
    title,
    geolocation: geolocationVar } = document.getData<DocumentData>();
  ...
  return (
    <div className={props.page.isPreview() ? 'has-edit-button' : ''}>
      ...
      { geolocationVar?.latitude && <p className="card-text">Latitude: {geolocationVar.latitude}</p> }
      { geolocationVar?.longitude && <p className="card-text">Longitude: {geolocationVar.longitude}</p> }
    </div>
  );
}
