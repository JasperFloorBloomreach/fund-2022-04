...
import training.standardfeatures.answer._02.NewsComponentInfo;
...
public class NewsComponent extends EssentialsNewsComponent {
    ...
    protected void contributeAndFilters(List<BaseFilter> filters, HstRequest request, HstQuery query) {
        ...
        NewsComponentInfo newsComponentInfo = getComponentParametersInfo(request);
        if (newsComponentInfo.getFilterNews() && newsComponentInfo.getTextToFilter() != null && !newsComponentInfo.getTextToFilter().isEmpty()) {
            try {
                Filter filter = query.createFilter();
                filter.addNotContains("myproject:title", newsComponentInfo.getTextToFilter());
                filters.add(filter);
